...
// other defines --------------------------------------
#define BRIGHTNESS   5     // values can be 0...15
...
int16_t  _dPosX = 0;                     //xPosition (display the date an time)
int16_t  _zPosX = 0;                     //xPosition (display the time)
boolean  _f_updown = false;              //scroll direction
uint16_t _chbuf[256];

// Font 4x8 for 8x8 matrix, 0,0 is above right
...
char2Arr_t(ans[20], _zPosX - 42, y);
char2Arr_t(ans[19], _zPosX - 42, y + y1);
char2Arr_t(':', _zPosX - 32, 0);
char2Arr_t(ans[17], _zPosX - 25, y);
char2Arr_t(ans[16], _zPosX - 25, y + y1);
String scTxt = ans.substring(0,16);

scrolltext(_dPosX, scTxt);
...
void setup() {
  ...
  //  wifiMulti.addAP("SSID", "PASSWORD");
   wifiMulti.addAP("Black Bok", "01914357821"); 
        ...
        if(httpCode > 0) {
          ...
          //  USE_SERIAL.printf("[HTTP] GET... code: %d\n", httpCode);
            ...
            if(httpCode == HTTP_CODE_OK) {
               ...
               ans = payload.substring(13,40);
               USE_SERIAL.print(ans);
			   display(); ///For Show in Display
                /*for(int i=13;i<40;i++)
                {
